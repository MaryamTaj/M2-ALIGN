"""Stage 1 evaluation on text-only multilingual benchmarks.

Loads the trained MindMerger (NLLB encoder → mapping → frozen Qwen3-VL) and
evaluates on the same text benchmarks as Baseline/evaluate_text.py.

Each example uses two inputs:
  - NLLB prefix: the raw source-language text (question / stem+options /
    premise+hypothesis), encoded by the MT encoder and projected by the
    mapping layer.
  - LLM prompt:  the same role-tagged, chat-formatted prompt as the
    Baseline (system + user messages via apply_chat_template), passed as
    input_ids_prompt to generate_from_mt.
...
def setup_logging(log_dir: str, task: str) -> logging.Logger:
    ...
    log_path = os.path.join(log_dir, f"stage1_eval_{task}_{timestamp}.log")
    logger = logging.getLogger(f"stage1_eval_{task}")
...
def load_model(
...
) -> tuple[MindMerger, AutoTokenizer, AutoTokenizer, torch.device, torch.dtype]:
    ...
    sig = inspect.signature(MindMerger.__init__)
...
def generate_text(
    ...
    # Tokenise source text for the NLLB prefix.
    tokenizer_mt.src_lang = nllb_code
    enc_mt = tokenizer_mt(nllb_text, truncation=True, max_length=max_mt_seq_len, return_tensors="pt")
    input_ids_mt = enc_mt["input_ids"].to(device)
    mask_mt = enc_mt["attention_mask"].to(device)

    # Build the role-tagged LLM prompt (identical to Baseline).
    messages = []
    if system_message:
    ...
    enc_llm = tokenizer_llm(formatted, return_tensors="pt", add_special_tokens=False)
    input_ids_prompt = enc_llm["input_ids"].to(device)
    mask_prompt = enc_llm["attention_mask"].to(device)

    gen_kw = dict(do_sample=False, max_new_tokens=max_new_tokens, eos_token_id=tokenizer_llm.eos_token_id)
    with torch.autocast(device_type="cuda", dtype=amp_dtype):
        return model.generate_from_mt(
            input_ids_mt, mask_mt, tokenizer_llm,
...
# ─── Scoring (identical to Baseline/evaluate_text.py) ───────────────────────
...
def _log_summary(
    ...
    logger.info("=== %s Stage1 Summary ===", task.upper())
    ...
    for lang in langs:
        ...
        for idx, sample in enumerate(tqdm(samples, desc=f"{task}/{lang}")):
            ...
            raw_out = generate_text(
                ...
                device, amp_dtype, TASK_MAX_NEW_TOKENS[task], max_mt_seq_len, _MATH_SYSTEM,
            )
...
def evaluate_xcsqa(
    ...
    for lang in langs:
        ...
        for idx, sample in enumerate(tqdm(samples, desc=f"x-csqa/{lang}")):
            ...
            raw_out = generate_text(
                prompt, nllb_text, nllb_code, model, tokenizer_mt, tokenizer_llm,
                device, amp_dtype, TASK_MAX_NEW_TOKENS["x-csqa"], max_mt_seq_len, _XCSQA_SYSTEM,
            )
...
def evaluate_nli(
    ...
    for lang in langs:
        ...
        for idx, sample in enumerate(tqdm(samples, desc=f"{task}/{lang}")):
            ...
            raw_out = generate_text(
                prompt, nllb_text, nllb_code, model, tokenizer_mt, tokenizer_llm,
                device, amp_dtype, TASK_MAX_NEW_TOKENS[task], max_mt_seq_len, _XNLI_SYSTEM,
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Stage 1 MindMerger evaluation on multilingual text benchmarks."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--mapping-ckpt",
        default="./outputs/MindMerger/nllb_corpus/mapping/pytorch_model.bin",
    )
